...
public class DesCryptService
{
    ...
    public BitArray EncryptBlock(BitArray source, BitArray key)
    {
        if (key.Length != KeySize)
            throw new ArgumentException($"Длина ключа должна быть равна {KeySize}.");
        if(source.Length != BlockSize)
            throw new ArgumentException($"Длина блока должна быть равна {BlockSize}.");
        LogTo?.Invoke($"Исходный массив: {source.BitArrayToString()}");
        LogTo?.Invoke($"Исходный ключ: {key.BitArrayToString()}\n");
        var subblock = source.Bisection();  //делим блок на полблоки LR
        var curKey = new BitArray(key); //текущий ключ для каждого раунда
        //16 раундов шифрования
        foreach (var shift in ShiftPattern) 
        {
            //циклически сдвигаем ключ согласно паттерну
            curKey = curKey.CycleShift(shift);
            //шифруем блок
            subblock = EncryptRound(subblock, curKey);
        }
        return BitArrayExtension.Compound(subblock);
    }
    public BitArray DecryptBlock(BitArray source, BitArray key)
    {
        if (key.Length != KeySize)
            throw new ArgumentException($"Длина ключа должна быть равна {KeySize}.");
        if (source.Length != BlockSize)
        ...
        {
            //дешифруем блок
            subblock = DecryptRound(subblock, curKey);
            //циклически сдвигаем ключ согласно паттерну
